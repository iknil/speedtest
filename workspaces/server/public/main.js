...
Core.prototype = {
   ...
   * @param {*} parameter string with the name of the parameter that you want to set
   ...
   * @param {*} server the server to be added as an object. Must contain the following elements:
   *  {
  ...
  start: function() {
    ...
    this.worker = new Worker("worker.js?r=" + Math.random());
...
function startOrStop() {
  if (core.getState() === 3) {
    ...
    $("startOrStopBtn").innerText = "Abort";
    core.onupdate = function(data){
      netData = data;
      callbackIDs.forEach(id => {
        sendMessage(`callback:id`, { statusCode: 0, msg: netData });
      });
    };
    core.onend = function(aborted){
      $("startOrStopBtn").innerText = "Start";
    };
    core.start();
  }
...
function updateUI() {
  if (netData !== null) {
    $("ping").innerText = `${netData.pingStatus || 0}ms`;
    $("jitter").innerText = `${netData.jitterStatus || 0}ms`;
    $("download").innerText = `${netData.dlStatus || 0}Mbps`;
    $("upload").innerText = `${netData.ulStatus || 0}Mbps`;
  }
}
...
$("startOrStopBtn").onclick = startOrStop;
frame(); 

var status = 0; // 0: init, 1: running, 2: end
// message sender
...
window.addEventListener('message', function(e) {
  ...
  switch(action) {
    case 'start':
      try {
        core.start();
        sendMessage('startCallback', { statusCode: 0, msg: 'started' })
      } catch(e) {
        sendMessage('startCallback', { statusCode: -1, msg: e.message });
      }
      break;
    case 'abort':
      try {
        core.abort();
        sendMessage('abortCallback', { statusCode: 0, msg: 'aborted' });
      } catch(e) {
        sendMessage('abortCallback', { statusCode: -1, msg: e.message });
      }
      break;
    case 'register':
      e.data.msg && e.data.msg.id && callbackIDs.push(e.data.msg.id); // register callback id
      break;
    case 'unregister':
      e.data.msg && e.data.msg.id && callbackIDs.splice(callbackIDs.indexOf(e.data.msg.id), 1); // unregister callback id
      break;
    default:
